...
class QuizQuestionsActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mQuestion = intent.getStringExtra(Constants.QUESTIONS)
        if (mQuestion == Constants.QUESTIONS_PHRASES) {
            mQuestionsList = Constants.getQuestionsPhrases()
        }
        else if (mQuestion == Constants.QUESTIONS_HARRY_POTTER) {
            mQuestionsList = Constants.getQuestionsHarry()
        }
        else if (mQuestion == Constants.QUESTIONS_HOGWARTS) {
            mQuestionsList = Constants.getQuestionsHogwarts()
        }
        else if (mQuestion == Constants.QUESTIONS_OTHER) {
            mQuestionsList = Constants.getQuestionsOther()
        }
    ...
    override fun onClick(v: View?) {
        when(v?.id) {
            ...
            R.id.btn_submit ->{
                if (!answerSelected) {
                    val toast = Toast.makeText(this, "Выбери ответ", Toast.LENGTH_LONG)
